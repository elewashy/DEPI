...
def delta_load():
    """
    Processes and loads new transaction data into the 'orders' table.
    """
    try:
        ...
        last_transaction_id = orders_cursor.fetchone()[0] or 0

        # Process all transaction files; the logic inside the loop will skip old ones
        for file in transaction_files:
            with open(file, 'r') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    try:
                        if int(row['transaction_id']) <= last_transaction_id:
                            continue
        ...
        print("Delta load completed successfully!")
